...
def main():
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers("tb_logs", experiment_config, init_kwargs={"wandb":{"name":args.run_name}})
                        ...
                        if hasattr(module, "weight"):
                            ...
                            try:
                                with warnings.catch_warnings():
                                    ...
                                    # accelerator.log(
                                    # f"{name}.weight_hist", wandb.Histogram(w), global_step=completed_steps
                                    # )
                            except:
                                logger.warn(
                    ...
                    for name, x in act_dict.items():
                        try:
                            with warnings.catch_warnings():
                                warnings.filterwarnings("ignore", category=DeprecationWarning)
                                # accelerator.log(
                                #     f"{name}.act_hist", wandb.Histogram(x), global_step=completed_steps
                                # )
                        except:
                            logger.warn(
